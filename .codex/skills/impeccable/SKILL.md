...
- **Project one-shot convention:** When a Korean request uses `점검` with a concrete UI URL, page, component, modal, drawer, or flow, route directly to `critique` and include its mandatory detector/browser evidence. Do not ask the user to choose between `critique` and `audit`. Use explicit `audit` only when the user names `audit` or asks specifically for technical/accessibility/performance compliance checks.
