...
test("accepts and normalizes Seoul and regional landline numbers", () => {
  for (const phone of ["02-1234-5678", "031-123-4567"]) {
    const result = validateEstimateDraft({ ...validDraft, phone }, "2026-08-05");
    assert.equal(result.success, true, phone);
    if (result.success) assert.equal(result.data.phone, phone.replace(/\D/g, ""));
  }
});
