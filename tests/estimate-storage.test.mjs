...
test("stores normalized estimates and private photo keys", async () => { const calls = []; const stored = await new PostgresEstimateStorage({ async unsafe(query, values) { assert.match(query, /\$9::text::jsonb/); calls.push(values); } }).save(draft, ["estimates/id/photo.jpg"]); assert.match(stored.reference, /^SC-\d{8}-[0-9A-F]{6}$/); assert.match(calls[0][8], /photo\.jpg/); });
