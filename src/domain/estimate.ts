...
export function validateEstimateDraft(input: Record<string, unknown>, today = new Date().toISOString().slice(0, 10)): EstimateValidationResult {
  const draft: EstimateDraft = { service: readText(input.service), name: readText(input.name), phone: readText(input.phone).replace(/\D/g, ""), area: readText(input.area), description: readText(input.description), preferredDate: readText(input.preferredDate), privacy: input.privacy === true };
  ...
  if (!/^0\d{9,10}$/.test(draft.phone)) fieldErrors.phone = "연락 가능한 휴대전화 번호를 입력해주세요.";
